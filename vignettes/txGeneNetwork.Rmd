...
abstract: |
  This workflow is a comprehensive guideline of how to construct a Pathway-Gene-Transcript network 
...
```{r install_dependencies, echo=FALSE, message=FALSE, warning=FALSE}
...
example_dataset <- read_csv("example_dataset.csv")
```
...
Here we add a centrality measure[ADD REFERENCE] to the network and size it accordingly using an `aes()` call inside `geom_node_point()`.
